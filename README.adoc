...
NOTE: This may want a `jenkins-agent-toggle.conf` file pointing to a
sufficiently privileged admin account `J_USER` (with its `J_PASS` token).
By default the account used for swarm agents is used, but for their work
it suffices to have permissions to create, configure and delete an agent,
but not necessarily to manipulate its metadata or live state (the Connect
and/or Disconnect permissions).
